	...
	upload_inp.addEventListener('change', async function () {
			...
			try {
					data = await instance.post('/upload_single_base64', {
							file: encodeURIComponent(BASE64), // 编码，防止特殊字符乱码
							filename: file.name
					}, {
							headers: {
					...
					// async await语法 这里throw 下面catch接
					throw data.codeText;
			} catch (err) {
...
(function () {
	let upload = document.querySelector('#upload3'),
			...
			upload_button_select = upload.querySelector('.upload_button.select'),
			upload_button_upload = upload.querySelector('.upload_button.upload'),
			upload_abbre = upload.querySelector('.upload_abbre'),   // 生成缩略图DOM
	...
	// 根据文件内容编译成buffer数据，生成hash名，同名不同图可以上传
	const changeBuffer = file => {
			return new Promise(resolve => {
					...
					fileReader.onload = ev => {
							...
							suffix = /\.([a-zA-Z0-9]+)$/.exec(file.name)[1];  // 以点什么什么结尾
							resolve({
									buffer,
									HASH,
									suffix,
	...
	upload_inp.addEventListener('change', async function () {
			...
			try {
					...
					data = await instance.post('/upload_single', formData, {
							// 文件上传中的回调函数 xhr.upload.onprogress，原生xhr对象中用这个监听
							onUploadProgress(ev) {
									let {
											loaded,
											total
...
(function () {
	...
	upload_list.addEventListener('click', function (ev) {
			let target = ev.target,
					...
					key;
			if (target.tagName === 'EM') {
					curLi = target.parentNode.parentNode; // 获得Li，是target的爷爷
					if (!curLi) return;
					upload_list.removeChild(curLi);  // DOM显示上移除
					// 数据中移除
	...
	const createRandom = () => {
			...
			return ran.toString(16).replace('.', '');  // 变成16进制的字符串，去掉点
	};
...
(function () {
	...
	// 拖拽获取 dragenter 进入容器触发 dragleave 离开容器触发 dragover 正在区域中触发 drop 放入容器触发
	...
	upload.addEventListener('dragover', function (ev) {
			ev.preventDefault();   // 阻止新窗口打开文件的默认行为
	});
...
(function () {
	...
	const changeBuffer = file => {
			return new Promise(resolve => {
					...
					fileReader.onload = ev => {
							...
							spark.append(buffer);
							HASH = spark.end();
							suffix = /\.([a-zA-Z0-9]+)$/.exec(file.name)[1];  // 后缀
							resolve({
									buffer,
									HASH,
									suffix,
	...
	upload_inp.addEventListener('change', async function () {
			...
			// 1. 固定切片数量
			// 2. 固定切片大小
			let max = 1024 * 100,  // 固定每个切片 1024 * 100 kb  
					count = Math.ceil(file.size / max),  // 算出切片的数量
					index = 0,
					chunks = [];
			// 大于100个 切片，则按照100个切
			if (count > 100) {
			...
			// file原型上有一个blob，上有一个slice方法，可以实现切片处理
			// file.slice(0, 1024)  从0b开始 切1024b file.slice(1024, 2048) 
			while (index < count) {
					chunks.push({
